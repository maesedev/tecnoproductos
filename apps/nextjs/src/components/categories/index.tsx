...
export default function Categorias() {
  ...
  const [selected, setSelected] = useState(ArrayCategorias[0])
    ...
    <Listbox value={selected} onChange={setSelected}>
      {({ open }) => (
        <>
          ...
          <Listbox.Label className="block text-lg text-center font-medium leading-6 text-gray-900">Categorias</Listbox.Label>
            ...
            >
              <Listbox.Options className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                ...
                {ArrayCategorias.map((item) => (
                  <Listbox.Option
                    key={item.name}
                    className=' text-slate-500 relative cursor-default select-none py-2 pl-3 pr-9'
                    value={item}
                  >
                    {({ selected, active }) => (
                      <>
